...
class Field:
    ...
    def start(self):
        ...
        for i in range(8):
            label = QLabel(ex)
            pixmap = QPixmap(f"chess_progect/{'black'}/{types_of_chess[i]}.png")
            label.setPixmap(pixmap)
        ...
        for i in range(1, 3):
            ...
            for j in range(8):
                label = QLabel(ex)
                pixmap = QPixmap(f"chess_progect/{color}/{'pawn'}.png")
                label.setPixmap(pixmap)
        ...
        for i in range(8):
            label = QLabel(ex)
            pixmap = QPixmap(f"chess_progect/{'white'}/{types_of_chess[i]}.png")
            label.setPixmap(pixmap)
